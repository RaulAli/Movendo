...
@Component({
  ...
  templateUrl: './admin_auth.page.html',
  styleUrls: ['./admin_auth.page.scss'],
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
...
export class AdminAuthPage implements OnInit {
  ...
  ngOnInit() {
    this.route.url.subscribe((data) => {
      ...
      this.title = this.authType === 'login' ? 'Iniciar sesión de administrador' : 'Registrar administrador';
  ...
  submitForm() {
    ...
    this.adminService.attemptAuth(this.authType, credentials).subscribe({
      next: (response) => {
        ...
        Swal.fire({
          icon: 'success',
          title: 'Éxito',
          text: this.authType === 'login' ? 'Inicio de sesión de administrador exitoso' : 'Registro de administrador exitoso'
        }).then(() => {
